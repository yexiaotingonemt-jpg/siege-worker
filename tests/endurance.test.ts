...
it('928-unit accumulation on the compact battlefield stays finite and collision-free',()=>{
 const s=new Simulation();s.start();s.god=true;s.buildings=[];s.reindex();
 s.time=900;s.tick(1/30);expect(s.enemies.length).toBe(928);expect(s.batches.every(b=>b.done)).toBe(true);
