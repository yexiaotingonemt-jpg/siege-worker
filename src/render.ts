...
export class BattleScene extends Phaser.Scene{
 ...
 enemy(g:Phaser.GameObjects.Graphics,e:Enemy){const d=ENEMIES[e.kind],x=e.x*TILE,y=e.y*TILE,scale=d.boss?Math.max(1.65,d.r/.6*1.65):1,bob=Math.sin(this.sim.time*9+e.id)*1.1;g.fillStyle(0x102018,.4).fillEllipse(x,y+9,22*scale,10*scale);const color=e.flash>this.sim.time?0xf5d7b4:d.color;
