...
it('a staffed fortress can fight the full independent 20-wave schedule to victory',()=>{
 ...
 expect(s.batches.every(b=>b.done)).toBe(true);expect(s.wave).toBe(20);expect(s.state).toBe('won');expect(s.stats.kills).toBe(1020);
},60000);
it('1020-unit accumulation on the compact battlefield stays finite and collision-free',()=>{
 const s=new Simulation();s.start();s.god=true;s.buildings=[];s.reindex();
 s.time=900;s.tick(1/30);expect(s.enemies.length).toBe(1020);expect(s.batches.every(b=>b.done)).toBe(true);
...
it('a remote client interpolates the 2131-enemy three-player schedule without rerunning battle AI',()=>{
 const host=new Simulation(3);host.start();host.god=true;host.buildings=[];host.reindex();const template=host.spawn('infantry',{x:24,y:16},20);host.enemies=Array.from({length:2131},(_,i)=>({...template,id:1000+i,x:2+(i%28),y:2+(Math.floor(i/28)%28)}));const fullBytes=JSON.stringify(host.networkState()).length,wire=JSON.stringify(host.networkState(true)),compactBytes=wire.length,guest=new Simulation(3);guest.start();guest.reconcileNetworkState(JSON.parse(wire),2);const hp=guest.enemies.reduce((sum,e)=>sum+e.hp,0),start=performance.now();
 for(let i=0;i<30;i++)guest.predictNetwork(1/30,2);
 const elapsed=performance.now()-start;console.log(JSON.stringify({test:'2131-unit remote prediction',oneSecondPredictionMs:Number(elapsed.toFixed(2)),fullBytes,compactBytes,remaining:guest.enemies.length}));
 expect(guest.enemies).toHaveLength(2131);expect(guest.enemies.reduce((sum,e)=>sum+e.hp,0)).toBe(hp);expect(compactBytes).toBeLessThan(fullBytes*.6);expect(elapsed).toBeLessThan(100);
},60000);
