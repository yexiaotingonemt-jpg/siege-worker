...
describe('realtime protocol validation',()=>{
 ...
 it('validates authoritative snapshot bounds and connected player count',()=>{
  ...
  expect(validNetworkState(state,3)).toBe(true);state.wave=21;expect(validNetworkState(state,3)).toBe(true);state.wave=22;expect(validNetworkState(state,3)).toBe(false);state.wave=0;state.players[2].x=999;expect(validNetworkState(state,3)).toBe(false);state.players[2].x=16.5;expect(validNetworkState(state,4)).toBe(false);state.gold=-1;expect(validNetworkState(state,3)).toBe(false);state.gold=120;state.players[1].gold=-1;expect(validNetworkState(state,3)).toBe(false);state.players[1].gold=120;state.players[1].xp=1;expect(validNetworkState(state,3)).toBe(false);state.players[1].xp=state.players[0].xp;state.players[1].level=2;expect(validNetworkState(state,3)).toBe(false);state.players[1].level=state.players[0].level;state.buildings.push({...state.players[0],owner:99});expect(validNetworkState(state,3)).toBe(false);
...
describe('client prediction reconciliation',()=>{
 ...
 it('predicts only the local worker and interpolates combat entities without running combat',()=>{
  const host=new Simulation(2);host.start();const enemy=host.spawn('heavy',{x:24,y:24},8),guest=new Simulation(2);guest.start();guest.spawn('heavy',{x:22,y:24},8);guest.reconcileNetworkState(JSON.parse(JSON.stringify(host.networkState())),1);
