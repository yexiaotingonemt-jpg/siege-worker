...
function renderOverlay(key:string){const root=$('overlay');root.innerHTML='';root.className=key?'visible '+(key==='menu'?'menu-overlay':''):'';if(!key)return;
 ...
 else if(key==='book')root.innerHTML=`<section class="modal book"><div class="book-head"><div><span class="eyebrow">FIELD MANUAL</span><h2>工匠的战地手册</h2></div><button data-action="book" class="close">×</button></div><div class="book-scroll"><div class="manual-grid"><article><h3>01 · 到现场，才能建造</h3><p>WASD / 方向键移动。数字1—5选择建筑，E在脚下开工。移动会中断，但保留进度；受伤不会清零。</p></article><article><h3>02 · 维护你的防线</h3><p>站上已完工建筑，R持续修复、F瞬间升级。修复付建造点。升级保持生命比例，不能代替修复。</p></article><article><h3>03 · 你是敌人的目标</h3><p>敌人优先攻击范围内的你，否则攻击附近建筑。建筑封路会被拆除。利用河流、石头和控制塔争取施工时间。</p></article><article><h3>04 · 时间不会等你</h3><p>每45秒来一波，旧敌人不会消失。5、10、15、20波出现BOSS。全部20波生成且清场后胜利。</p></article><article><h3>05 · 只带两件主动</h3><p>BOSS掉落道具，走到所在格按G拾取。Q / 空格使用主动。满格时替换，旧装备留在原地，冷却不会重置。</p></article><article><h3>06 · 祝福伴你同行</h3><p>被动拾取即成为整局BUFF，不占格。建筑需要在你周围5格内才能受益。无敌只保护你，工地仍会被攻击。</p></article></div><h3 class="catalog-title">主动装备 · 9种</h3><div class="catalog">${Object.entries(ACTIVES).map(([k,d])=>`<article><span>${d.icon}</span><div><b>${d.name}</b><p>${d.desc} · 冷却${d.cd}秒</p></div>${sim.practice?`<button data-action="grant" data-kind="${k}">投放</button>`:''}</article>`).join('')}</div><h3 class="catalog-title">被动祝福 · 11种</h3><div class="catalog">${Object.entries(BUFFS).map(([k,d])=>`<article><span>${d.icon}</span><div><b>${d.name}</b><p>${d.desc}</p></div>${sim.practice?`<button data-action="grant" data-kind="${k}">投放</button>`:''}</article>`).join('')}</div></div></section>`;
}
function drawMini(){const c=$('minimap') as HTMLCanvasElement,ctx=c.getContext('2d')!,scale=c.width/SIZE;ctx.fillStyle='#233b30';ctx.fillRect(0,0,c.width,c.height);
 for(let i=0;i<sim.map.tiles.length;i++){const t=sim.map.tiles[i];if(t){ctx.fillStyle=t===1?'#627268':'#487d7e';ctx.fillRect(i%SIZE*scale,Math.floor(i/SIZE)*scale,scale,scale);}}
 ctx.fillStyle='#e7cb8b';for(const b of sim.buildings)ctx.fillRect(b.x*scale-1,b.y*scale-1,2,2);
 ctx.fillStyle='#dc8d79';for(const e of sim.enemies)ctx.fillRect(e.x*scale-1,e.y*scale-1,ENEMIES[e.kind].boss?4:1.5,ENEMIES[e.kind].boss?4:1.5);
