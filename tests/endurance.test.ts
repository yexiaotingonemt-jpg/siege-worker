...
it('a staffed fortress can fight the full independent 21-wave schedule to victory',()=>{
 ...
 console.log(JSON.stringify({test:'21-wave endurance',time:s.time,state:s.state,kills:s.stats.kills,remaining:s.enemies.map(e=>({kind:e.kind,x:e.x,y:e.y,d:distance(e,s.player),target:e.target,wind:e.wind,skillAt:e.skillAt,stunUntil:e.stunUntil})),zones:s.zones,maxEnemies,maxBosses,elapsedMs:Math.round(performance.now()-start)}));
 expect(s.batches.every(b=>b.done)).toBe(true);expect(s.wave).toBe(21);expect(s.state).toBe('won');expect(s.stats.kills).toBe(977);
},60000);
it('977-unit accumulation on the compact battlefield stays finite and collision-free',()=>{
 const s=new Simulation();s.start();s.god=true;s.buildings=[];s.reindex();
 s.time=1000;s.tick(1/30);expect(s.enemies.length).toBe(977);expect(s.batches.every(b=>b.done)).toBe(true);
...
it('a remote client interpolates the 2045-enemy three-player schedule without rerunning battle AI',()=>{
 const host=new Simulation(3);host.start();host.god=true;host.buildings=[];host.reindex();const template=host.spawn('infantry',{x:24,y:16},21);host.enemies=Array.from({length:2045},(_,i)=>({...template,id:1000+i,x:2+(i%28),y:2+(Math.floor(i/28)%28)}));const fullBytes=JSON.stringify(host.networkState()).length,wire=JSON.stringify(host.networkState(true)),compactBytes=wire.length,guest=new Simulation(3);guest.start();guest.reconcileNetworkState(JSON.parse(wire),2);const hp=guest.enemies.reduce((sum,e)=>sum+e.hp,0),start=performance.now();
 for(let i=0;i<30;i++)guest.predictNetwork(1/30,2);
 const elapsed=performance.now()-start;console.log(JSON.stringify({test:'2045-unit remote prediction',oneSecondPredictionMs:Number(elapsed.toFixed(2)),fullBytes,compactBytes,remaining:guest.enemies.length}));
 expect(guest.enemies).toHaveLength(2045);expect(guest.enemies.reduce((sum,e)=>sum+e.hp,0)).toBe(hp);expect(compactBytes).toBeLessThan(fullBytes*.6);expect(elapsed).toBeLessThan(100);
},60000);
